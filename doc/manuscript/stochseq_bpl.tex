...
\com{\jwm{sequencing used twice}}
,
however 
\com{\jwm{Avoid using ``however'' unless there is a real contradiction. Moreover, try to avoid the boilerplate ``X is important, however aspect Y is not understood'' -- too many generations of stumped writers have done this before you.}}
backwards motion of the DNA molecule due to stochastic motion 
\com{\jwm{this is both a circular argument and a repetition of words}}
inside the pore is a novel source of error 
\com{\jwm{Try to avoid saying ``novel'' -- truly novel things don't need to be explicitly labeled as such, and with not-so-novel things it only looks disingenuous}}
and presents a barrier to high accuracy reads.
We present a theoretical model
\com{\jwm{no. we present an analysis technique. a biased random walk is not a serious model of the physics in this system.}}
...
\com{\jwm{
I think you can do much better than this. I'm guessing you wrote this quickly, and it shows. An abstract should not so much say what you've done as let the reader know why they should read the paper. Try to establish the problem more clearly and really drive home the point that statistical inference allows you to overcome read-length barriers that arise from diffusivity. }}
}
...
\com{\jwm{this sentence does not parse, probably because there is one ``high(est) coverage'' too many}}
...
\com{\jwm{break this sentence into two. }}
...
\com{\jwm{This last sentence needs to be more specific.}}
...
\com{\jwm{I think this is a really good paragraph.}}
...
\jwm{In this letter, we consider a simple physical model of DNA translocation as a one-dimensional biased random walk and analyze how this diffusive motion affects the difﬁculty achiev¬able read accuracy of nanopore sequencing. To do so, we simulate noisy data where the true sequence is unknown, and develop a statistical technique to estimate the most likely DNA sequence associated with the simulated read signal. Our method is based on Hidden Markov Models (HMMs), which have recently been used to study possible multi-base resolution in a nanopore sequencer \cite{Timp:2012}, but have not yet been applied to the problem of diffusive motion inside the pore. This work shows that combining multiple reads from a given input DNA sequence passing through a nanopore allows accurate estimates of the sequence, both in the presence of highly diffusive molecular motion and at high base-call error rates.}
\com{\jwm{This paragraph was a complete mess and basically said everything twice. I've tried to restructure and get rid of redundancies.}}
...
A \jwm{schematic} representation of \jwm{of a nanopore sequencing device} is shown in Figure \ref{fig:fig1}A.
...
\begin{figure}%\vspace*{3pt}
  ...
  \caption{A. \jwm{Schematic representation} of a nanopore sequencing device. B. Transit time distribution \jwm{of} a set of random walks for different forward biases (\jwm{sequence length $L=100$}).}
  \label{fig:fig1}
\end{figure}

Given this set of $N$ read sequences, the statistical task is to infer the \jwm{\st{true}} sequence most likely to have generated the observed data \com{\jwm{``true sequece'' or ``most likely sequence'', pick one}}.
...
\end{equation}
\com{\jwm{you don't explain how the error rate $e$ is rolled into $S_{dl}$. this is almost guaranteed to confuse your reviewers.}}
...
We maximize the \jwm{\st{complete-data}} likelihood, \jwm{$p(X \:|\: \theta)$}, with respect to the model parameters using an implementation of \jwm{the} expectation-maximization (EM) \cite{Baum:1970}.
\com{\jwm{the EM procedure maximizes $p(X \:|\: \theta)$ not $p(X,Z \:|\: \theta)$. basic mistakes like these should not be in the draft at this point.}}
Because the \jwm{joint probability of data and states can be written as a product over} independent output reads
\com{\jwm{likelihood $\not=$ joint}}
...
\com{\jwm{the factorization of the joint over all time series is an assumption, not a given, and should be described as such}}
This \jwm{shared parameter estimation scheme allows us to incorporate} all the information provided in the reads while allowing an efficient, parallel calculation.
...
The final inference accuracy is measured as the Levenstein distance \com{\jwm{citation?}} between the input sequence and inferred sequence, normalized by $L$.
...
Inference errors in this model are generally associated with increases in the inference entropy \com{\jwm{``inference entropy'' is not a thing as far as I know -- you should define what you mean by entropy}}, suggesting a possible approach that further analysis might systematically identify and remove the inference errors due to bidirectional motion.
...
Under this model, reasonably accurate sequence inference is achievable even within the experimental constraints of today's nanopore sequencers\jwm{\st{.}} ($p \approx 0.7$).
...
\com{\jwm{I think this is a pretty strong closing paragraph. Try to make your abstract sound this good.}}
