...
To date, a significant obstacle of \jwm{nanopore approaches} has been overcoming the fast stochastic motion of the individual molecules as they are driven through the pore \cite{Venkatesan:2011, Lu:2011}.
...
% \jwm{The expected length $T$ depends on the forward bias.} 
% \jwm{In the limit $\bias \to 1$ there is no backward motion and $T \to L$.}
% \jwm{For purely diffusive motion $\bias \to 1/2$ and $T \to \pi^2 L / 4$.}
$N$ is a notion of sequence coverage in terms of identical molecules sequenced.
...
\end{equation}
\jwm{\com{ok there actually is something wrong here. In the limit $\bias \to 1$, the read length converges to $T \to L$. Conversely, in the limit $\bias \to 1/2$ however, the length tends to $T \to \pi^2 L^2 / 4$. Does this relationship follow from a linear expansion around $\bias = 1/2$ ?}}
...
\jwm{\com{note: this $O(L)$ vs $O(L^2)$ argument needs to be consistent with equation \ref{eq:expected-T}}}
...
For \emph{N}=1, the average inference accuracy goes roughly as {\raise.17ex\hbox{$\scriptstyle\sim$}}\jwm{$\bias$}, a baseline measure of performance (Fig. \ref{fig:parameter_sweeps}\emph{~A}).
...
\jwm{The source code used to perform the analysis in this paper will shortly be available via http://stochseq.github.io}

\section*{ACKNOWLEDGEMENTS}

The authors \jwm{gratefully acknowledge helpful} discussions with David Blei and David Pfau. 
\jwm{KJE and CHW were supported by NIH grant U54-CA121852 (National Center for Multiscale Analysis of Genomic and Cellular Networks).}
\jwm{JWM was supported through the NWO Rubicon Fellowship 680-50-1016.}
